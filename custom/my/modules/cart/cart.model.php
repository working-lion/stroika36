...
class Cart_model extends Model
{
	...
	public function form_table()
	{
		...
		foreach ($this->result["delivery"] as &$row)
		{
			...
			if($row["service"])
			{
				...
				$row["service_info"] = ($_SESSION["cart_delivery"] == $row["id"] && ! empty($_SESSION["cart_delivery_info"]) ? $_SESSION["cart_delivery_info"] : '');
				$row["service_price"] = ($_SESSION["cart_delivery"] == $row["id"] && ! empty($_SESSION["cart_delivery_summ"]) ? $_SESSION["cart_delivery_summ"] : '');
			}
	...
	private function form_param($one_click = false)
	{
		...
		if($this->diafan->configmodules('subscribe_in_order', 'subscribtion'))
		{
	...
	private function yandex_fast_order()
	{
		...
		$this->result["yandex_fast_order_link"] =  'http://market.yandex.ru/addresses.xml?callback='
