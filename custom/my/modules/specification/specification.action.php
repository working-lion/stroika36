...
class Specification_action extends Action
{
            ...
            if(!$row) {
                ...
                if($rows) {
                    ...
                    foreach ($rows as $row) {
                        ...
                        $arrorAdd = $this->add_to_cart($good_id, $count, array());

                        if( ! $arrorAdd) {
                            $this->result["errors"][0] = 'Ошибка при добавлении товара с ID=' . $good_id . '. Обратитесь к администратору сайта.';
                            $this->result["result"] = "success";
                            return;
                            break;
                        }
    ...
    public function add_to_cart($good_id, $count = 1, $additional_costs = Array())
    {
        ...
        if(! empty($additional_costs))
        {
            ...
            if($a_cs)
            {
                $additional_cost_arr = DB::query_fetch_value(
                    "SELECT additional_cost_id FROM {shop_additional_cost_rel}"
                . " WHERE element_id=%d AND trash='0' AND additional_cost_id IN (%s)",
                $good_id,
                implode(',', $a_cs), "additional_cost_id");
            }
        }

        $additional_cost_arr_2 = DB::query_fetch_value(
            "SELECT r.additional_cost_id"
            . " FROM {shop_additional_cost_rel} AS r INNER JOIN {shop_additional_cost}"
            . " AS a ON a.id=r.additional_cost_id WHERE r.element_id=%d AND r.trash='0' AND a.required='1'",
            $good_id,
            "additional_cost_id");
