...
echo '<div class="h2">'.$this->diafan->_('Технические характеристики').'</div>';
echo '<table class="shop_params"><tbody>';
foreach ($result["rows"] as $param)
{
    if($param['id'] == 37) continue; // пропускаем количество

	echo '<tr class="shop_param__item param'.$param["id"].'">';

	echo '<td class="shop_param_tit '.($param["type"] == 'title' ? 'shop_param_title' : '').'">'.$param["name"] . ':</td>';
	echo '<td>';
	if ($param["value"])
	{
