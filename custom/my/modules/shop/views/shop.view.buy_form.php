...
if ($result["row"]["price_arr"]) {
    foreach ($result["row"]["price_arr"] as $price) {
        ...
        echo '<span class="price"><span class="js_shop_price" summ="' . $price["price_no_format"] . '" format_price_1="' . $this->diafan->configmodules("format_price_1",
                "shop") . '" format_price_2="' . $this->diafan->configmodules("format_price_2",
                "shop") . '" format_price_3="' . $this->diafan->configmodules("format_price_3",
                "shop") . '">' . $price["price"] . '</span> ' . $result["result"]["currency"] . '<div class="title_price">' . $p32name . '</div>';
        //<span class="title">' . $this->diafan->_('Цена ') . '</span>
        if ( !empty($price["old_price"])) {
            echo '<span class="shop_old_price price-old"><span class="shop_price_value strike">' . $price["old_price"] . ' ' . $result["result"]["currency"] . $p32name . '</span></span>';
        }
            ...
            if (count($result["row"]["param_multiple"][$param["id"]]) == 1) {
                ...
                foreach ($param["values"] as $value) {
                    if ( !empty($result["row"]["param_multiple"][$param["id"]][$value["id"]])) {
                        if ( !$select) {
                            if ($param["id"] == 9) {
                                $select = '<select style="display:none;" name="param' . $param["id"] . '" class="shop-dropdown dropdown inpselect' . ($result["row"]["param_multiple"][$param["id"]][$value["id"]] == 'depend' ? ' depend_param js_shop_depend_param': '') . '">';
                            } else {
                                $select = '<span class="title_select hide"> ' . $param["name"] . '</span> <select name="param' . $param["id"] . '" class="shop-dropdown dropdown inpselect' . ($result["row"]["param_multiple"][$param["id"]][$value["id"]] == 'depend' ? ' depend_param js_shop_depend_param': '') . '">';
                            }
                        }
...
echo '<div class="summa_buy hide">Итого: <div class="buy_form_itogo_block"></div></div>';

if (empty($hide_submit)) {
    echo '<input class="btn btn_buy" type="button" class="button solid" value="' . $this->diafan->_('В корзину',
            false) . '" action="buy">';
...
if (empty($hide_submit) && !empty($result["result"]["one_click"])) {
    echo '<span class="js_shop_one_click shop_one_click"><input type="button" class="btn_one_click btn_trans" value="' . $this->diafan->_('Купить в 1 клик',
            false) . '" action="one_click"></span>';
}
...
if ( !empty($result["row"]["count_in_cart"])) {
    ...
    echo '>' . $this->diafan->_('В <a href="%s">корзине</a> %s %s', true,
            BASE_PATH_HREF . $result["result"]["cart_link"], $result["row"]["count_in_cart"], $measure_unit);
} else {
