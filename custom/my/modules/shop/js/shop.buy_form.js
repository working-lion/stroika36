...
$(document).on('change', "input[name=count]", function() {
	calc_buy_form_itogo($(this).parents('form'));
});
...
function calc_buy_form_itogo(th)
{
	var param_code = '';
	var p32id=0;
	$(".js_shop_depend_param, .depend_param", th).each(function(){
		if ($(this).attr('name')!='param32')
	...
	if($('.js_shop_param_price, .shop_param_price', th).length)
	{
		...
		{
			var pb=$('.js_shop_param_price' + param_code + '[p32id='+p32id+'], .shop_param_price'+param_code+'[p32id='+p32id+']', th);
			var c=parseInt($(th).find('input[name=count]').val());
			var p=parseFloat($(pb).attr('price_no_format'))*c;
			$('.buy_form_itogo_block',$(th).closest('form')).html(format_price(p,2) + ' ' +$(pb).attr('currency'));
		}
	}
}
