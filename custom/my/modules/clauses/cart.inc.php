...
class Cart_inc extends Diafan
{
	...
	public function set($value = array(), $id = 0, $param = false, $additional_cost = false, $name_info = '')
	{
		...
		if (!empty($params[31]))
		{
			...
			elseif($name_info == "count")
			{
				$cc = preg_replace('/[^0-9\.\-]+/', '', $value);
			}
			if ($cc<1)
				$cc=1;
			$params[31]=DB::query_result("SELECT ps.id FROM {shop_param_select} as ps INNER JOIN {shop_price} as p ON p.good_id=%d AND p.trash='0' INNER JOIN {shop_price_param} as pp ON pp.param_id=ps.param_id AND pp.price_id=p.id AND pp.param_value=ps.id AND pp.trash='0' WHERE ps.param_id=31 AND CAST(ps.[name] AS UNSIGNED INTEGER)<=%d and ps.trash='0' ORDER BY CAST(ps.[name] AS UNSIGNED INTEGER) DESC LIMIT 1",$id,$cc);
			asort($params);
			$tmp_param = serialize($params);
			if ($tmp_param!=$param && !empty($this->cart[$id]) && !empty($this->cart[$id][$param]))
			{
				$this->cart[$id][$tmp_param]=$this->cart[$id][$param];
